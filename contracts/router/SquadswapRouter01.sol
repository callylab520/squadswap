...
contract SquadswapRouter01 is ISquadswapRouter01 {
    address public immutable override factory;
    address public immutable override WETH;

    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'SquadswapRouter01: EXPIRED');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'SquadswapRouter01: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = SquadswapLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'SquadswapRouter01: INSUFFICIENT_A_AMOUNT');
 ...
 // **** REMOVE LIQUIDITY ****
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, 'SquadswapRouter01: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'SquadswapRouter01: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            ISquadswapPair(SquadswapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, SquadswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'SquadswapRouter01: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, SquadswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        _swap(amounts, path, to);
    }
    ...
    {
        require(path[0] == WETH, 'SquadswapRouter01: INVALID_PATH');
        amounts = SquadswapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'SquadswapRouter01: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'SquadswapRouter01: INVALID_PATH');
        amounts = SquadswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'SquadswapRouter01: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, SquadswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'SquadswapRouter01: INVALID_PATH');
        amounts = SquadswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'SquadswapRouter01: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(SquadswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure override returns (uint amountIn) {
        return SquadswapLibrary.getAmountOut(amountOut, reserveIn, reserveOut);
    }
